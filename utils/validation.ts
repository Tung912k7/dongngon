 ...
 */
export function validatePoeticForm(
  content: string,
  hinhThuc: string,
  writingRule?: string
  ...
  const normalizedWritingRule =
    writingRule === "1 câu" || writingRule === "sentence"
      ? "sentence"
      : writingRule
        ? "sentence"
        : undefined;
  ...
  const requiredWords =
    POETIC_FORM_LIMITS[hinhThuc] ||
    (hinhThuc.includes("Thơ") && !hinhThuc.includes("tự do")
      ? parseInt(hinhThuc.replace(/[^0-9]/g, ""))
      : null);
